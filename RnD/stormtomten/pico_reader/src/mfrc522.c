...
void rfid_init(uint32_t spi_baud, spi_inst_t *spix, uint8_t sck_pin,
               uint8_t mosi_pin, uint8_t miso_pin, uint8_t cs_pin,
               uint8_t rst_pin) {
  /*
          if(spix == 0)
...
uint8_t rfid_transceive(uint8_t *send_data, uint8_t send_len,
  ...
  write_reg(CommIEnReg,
            irq_en | 0x80);    // Enable IRQ + global IRQ enable - CommIEnReg
...
uint8_t card_command(uint8_t command, uint8_t *send_data, uint8_t send_len,
...
uint8_t rfid_anticoll(uint8_t *data, uint8_t *len) {
  ...
  // PICC_ANITCOLL, NVB
  // NVB = number of valid bits, with 0x93, 0x20 : 5-bytes (4-uid + 1 bcc
  // checksum) 0x93 : 4-byte uid, 0x95 : 7-byte uid, 0x97 : 10-byte uid
  ...
  uint8_t status =
      card_command(PCD_TRANSCEIVE, buf, buflen, back_data, &back_data_len);
...
uint8_t rfid_get_sak(uint8_t *atqa, uint8_t *uid) {
  ...
  // command + crc
  uint8_t command[9] = {0x93 /*PICC_SELECTTAG*/,
                        0x70,
                        uid[0],
                        uid[1],
                        uid[2],
...
uint8_t rfid_auth(uint8_t picc_auth_mode, uint8_t sector, uint8_t *key,
                  uint8_t *uid) {
  uint8_t status;
...
uint8_t rfid_get_uid(uint8_t *uid, uint8_t *uid_len) {
  uint8_t status;
  uint8_t tag_type[2] = {0};

  // Step 1: Send REQA or WUPA to detect a card
  status = rfid_request(REQ_REQA, tag_type);
  if (status != MI_OK) {
    // printf("rfid_get_uid: No card detected\n");
    return MI_ERR;
  }

  // Step 2: Perform anticollision to get the UID
  status = rfid_anticoll(uid, uid_len);
  if (status != MI_OK ||
      *uid_len != 5) { // Expecting 5 bytes (4-byte UID + 1-byte BCC)
    // printf("rfid_get_uid: Anticollision failed or invalid UID length\n");
    return MI_ERR;
  }

  // Verify BCC (Block Check Character): XOR of the 4 UID bytes
  uint8_t bcc = uid[0] ^ uid[1] ^ uid[2] ^ uid[3];
  if (bcc != uid[4]) {
    // printf("rfid_get_uid: BCC verification failed\n");
    return MI_ERR;
  }

  // UID is valid, return success
  return MI_OK;
}
